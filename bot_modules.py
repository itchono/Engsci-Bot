...
class Listener(commands.Cog):
    ...
    async def on_member_join(self, user: discord.Member):
        ...
        # as a failsafe, these will default to the first channel in the server
        # if nothing is found.

        if not (rule_ch := discord.utils.get(
                user.guild.channels, name='rules')):
            rule_ch = user.guild.channels[0]

        if not (introductions_ch := discord.utils.get(
                user.guild.channels, name='introduce-yourself')):
            introductions_ch = user.guild.channels[0]

        if not (main_ch := discord.utils.get(
                user.guild.channels, name='arrivals')):
...
class RoleManager(commands.Cog):
    ...
    async def on_ready(self):
        if not (g := self.bot.get_guild(int(cfg["Settings"]["guild"]))):
            g = self.bot.guilds[0]
        self.guild = g
        logger.info(f"Role manager loaded for: {g}")
    ...
    async def pronouns(self, ctx: commands.Context, *, pronouns):
        ...
        * other
        ...
        if pronouns in currently_existent_roles:
            roles = set(u.roles)
            # if they want to change pronouns
            roles -= set([pronoun_role(i) for i in currently_existent_roles])
